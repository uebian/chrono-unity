...
public class UChRichTextEditorCore
{
    ...
    public int MeasureColumn(int lineIndex, float xPos, GUIStyle style)
    {
        ...
        float firstCharLeft = style.CalcSize(new GUIContent("")).x;  // typically 0
        ...
        {
            float leftBoundary = style.CalcSize(new GUIContent(line.Substring(0, i))).x;
            float rightBoundary = style.CalcSize(new GUIContent(line.Substring(0, i + 1))).x;
        ...
        // If xPos is beyond the last character, place caret at end of line
        return length;
    }
    ...
    private bool IsPurelyNumericArray(string text, int openPos, int closePos)
    {
        ...
        try
        {
            var arr = JToken.Parse(candidate) as JArray;
    ...
    private string BuildSingleLineArray(string text, int openPos, int closePos)
    {
        ...
        // or [1.0, 2.5] etc.
        string candidate = "[" + inner + "]";
        var arr = JToken.Parse(candidate);

        // Single-line serialization
        string singleLine = arr.ToString(Newtonsoft.Json.Formatting.None);
